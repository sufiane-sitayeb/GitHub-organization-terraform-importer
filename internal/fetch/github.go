...
// UsersT This struct hold only the informations we need for the users
type UsersT struct {
	GithubModuleName string
	Login            string
	Role             string
...
func Users(githubOrg string, ctx context.Context, client *github.Client) (users []UsersT) {
	...
	for _, role := range memberOptions {
		...
		for {
			users, resp, err := client.Organizations.ListMembers(ctx, githubOrg, opt)
			if err != nil {
				fmt.Printf("ERROR: %s\n", err)
				os.Exit(1)
			}
			memberByRole[role] = append(memberByRole[role], users...)
			if resp.NextPage == 0 {
				break
			}
			opt.Page = resp.NextPage
		...
		// Extract from memberByRole only the informations we need

		for _, user := range memberByRole[role] {

			// Members holds only the informations we need for the template.
			users = append(users, UsersT{
				GithubModuleName: strings.Replace(user.GetLogin(), " ", "_", -1),
				Login:            user.GetLogin(),
				Role:             role,
			})
		}
